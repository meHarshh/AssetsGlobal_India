...
public class HomeLoanServiceImpl implements HomeLoanService {
	...
	// this method is used to add lead who want to puchase the property with the
	// home loan
	@Override
	...
	private HomeLoanResponse mapToResponse(HomeLoan homeLoan) {
		...
		homeLoanResponse.setLoadId(homeLoan.getLoadId());
		homeLoanResponse.setName(homeLoan.getName());
		homeLoanResponse.setEmail(homeLoan.getEmail());
		homeLoanResponse.setPhoneNumber(homeLoan.getPhoneNumber());
		homeLoanResponse.setBankingPartner(homeLoan.getBankingPartner());
